...
void	MiscTest()
{
	BaseArrayDemo();																					// demonstrates basic use of the arrays
	GenericArrayDemo();																				// shows that BaseArray, BlockArray, PointerArray etc. are interchangable
	PointerArraySpecificDemo();																// just a few methods specific to the PointerArray
	SortedArrayDemo();																				// how to use SortedArray
	ListGetNextDemo();																				// how to add old style GetNext()/GetPrev() functionality to a list element
}
...
{
public:
	DemoElement(LONG a = 1, LONG b = 2, LONG c = 3, LONG d = 4) : _a(a), _b(b), _c(c), _d(d)
	{
	}
...
struct MySortedIntegerArray : public SortedArray<MySortedIntegerArray, BaseArray<VLONG> >
{
	// your sorted array must implement a LessThan() method
	static inline c4d_misc::Bool LessThan(VLONG a, VLONG b)	{ return a < b; }
};
...
static void	SortedArrayDemo()
{
	...
	for (VLONG i = 1; i < test.GetCount(); i++)
	{
		if (test[i] < test[i - 1])
			GeBoom();
	}
