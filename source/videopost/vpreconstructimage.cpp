...
{
	public:
		static NodeData *Alloc(void) { return gNew ReconstructData; }
...
{
	if (vps->vp!=VIDEOPOSTCALL_RENDER || vps->open || *vps->error!=RENDERRESULT_OK || vps->thread->TestBreak()) return RENDERRESULT_OK;
	...
	for (y=y1;y<=y2;y++)
	{
		...
		for (b=buffer,x=x1;x<=x2;x++,b+=cpp, ind++)
		{
			...
			if (!(*ind)) continue; // unchanged background pixel
			col/=SCO 256.0;
			b[0]=col.x;
			b[1]=col.y;
			b[2]=col.z;
...
Bool ReconstructData::RenderEngineCheck(BaseVideoPost *node, LONG id)
{
